<?php
namespace Corporate\PaySDK\Base;

use Corporate\PaySDK\Utils\RSAEncryptor;
use GuzzleHttp\Psr7\Stream;
use Psr\Http\Message\ResponseInterface;
use Corporate\PaySDK\Http\HttpClient;
use Corporate\PaySDK\Http\HttpRequest;
...
class CorporateClient{
    ...
    private $corporatePublicKey;

    private $corporatePrivateKey;

    private $RSAEncryptor;

    const DEFAULT_CHARSET = "UTF-8";
    const SDK_VERSION = "V1.0";
    ...
    public function __construct(Config $config)
    {
        ...
        $this->corporatePrivateKey = $this->getPrivateKey($this->getConfig("corporatePrivateCertPath"));
        $this->corporatePublicKey = $this->getPublicKey($this->getConfig("corporatePublicCertPath"));
    }
    ...
    public function buildPostParams($params){
    	...
    	$post_params = [
    	    "data" => $encrypted,
    	    'sign' => $this->sign($encrypted,$this->corporatePrivateKey),
    ...
    public function rsaEncrypt($data){
        return $this->RSAEncryptor->pubEncrypt($data,$this->corporatePublicKey);
    }

    public function rsaDecrypt($data){
        return $this->RSAEncryptor->privDecrypt($data,$this->corporatePrivateKey);
    }
    ...
    public function verify($data_arr)
    {
        ...
        return $this->RSAEncryptor->verify($content, $sign, $this->corporatePublicKey);
    }
